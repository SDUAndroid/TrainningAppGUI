...
public class LoginDataSource
{
	...
	public void addLogin(Login login)
	{
		...
		// Inserting Row
		database.insert(MySQLiteHelper.TABLE_LOGIN, null, values);
		database.close();
	}
	...
	{
		Cursor cursor = database.query(MySQLiteHelper.TABLE_LOGIN, new String[] {
				MySQLiteHelper.COLUMN_IDUSER, MySQLiteHelper.COLUMN_USER, MySQLiteHelper.COLUMN_PASSWORD },
				MySQLiteHelper.COLUMN_USER + "=?", new String[] { String.valueOf(user) }, null, null, null,
				null);
		if (cursor != null)
			cursor.moveToFirst();
		Login login = new Login(Integer.parseInt(cursor.getString(0)), cursor.getString(1),
				cursor.getString(2));
		return login;
	}
